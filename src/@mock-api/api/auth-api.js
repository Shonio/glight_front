...
import userApi from '../data/user-data.json';

let usersApi = userApi.auth_users.value;

/* eslint-disable camelcase */

mock.onGet('/api/auth/sign-in').reply(async (config) => {
    const {phone, password} = JSON.parse(config.data);
    const user = _.cloneDeep(usersApi.find((_user) => _user.info.phone === phone));
...
mock.onPost('/api/auth/sign-up').passThrough();
